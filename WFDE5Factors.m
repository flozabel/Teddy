...
function WFDE5Factors(parameters_out,path_wfde5,outdir,startyear,endyear,lat_global,lon_global,res,global_mask,parallel_cpus)
...
for parameterloop=1:length(parameters_out)
  ...
  write_netcdf_dailyval(dailyval,lat_global,lon_global,timestamps,var_name,var_name_long,unit_ncdf,time_unit,filename,comment); %save as netcdf
  clear('dailyval');
  ...
  if(strcmpi(parameter_out,'tas')==1)
    ...
    write_netcdf_dailyval(dailyval_min,lat_global,lon_global,timestamps,var_name,var_name_long,unit_ncdf,time_unit,filename,comment); %save as netcdf
    var_name='max';
    var_name_long='daily max value';
    unit_ncdf=[unit_out];
    filename=[outdir,filesep,parameter_out,'_WFDE5_max_doy_',num2str(startyear),'-',num2str(endyear)];
    write_netcdf_dailyval(dailyval_max,lat_global,lon_global,timestamps,var_name,var_name_long,unit_ncdf,time_unit,filename,comment); %save as netcdf
